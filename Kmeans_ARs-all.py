...
for alx, alg in enumerate(algorithms):
    ...
    total_scaled.append(scaled_data)


new_scaled_data = np.concatenate(  total_scaled )
...
silhouette_scores = compute_silhouette_scores(K, new_scaled_data)
...
clustered_data = kmeans.fit_predict(new_scaled_data)
...
with open(f'{cluster_path}cluster_values_all_ardts', 'w') as output:
