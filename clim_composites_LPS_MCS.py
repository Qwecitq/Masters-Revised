...
final_path = f'{sv_path}/{reg}_ARS_DJF_comps_{algo}_{vb}_anoms/'
...
def process_year(r):
    ...
    clim_pths = glob.glob(f'../SWA_work/climatologies_1980-2020/djf_*{vb}*',recursive=True)        #load climatologies 
    ...
    if typ == 'levels' or typ== 'pot':
        ...
        clim = xr.open_mfdataset(clim_pths).sel(level=[850,700,500,300,250]).mean(meaner)
        print(clim)
        ds = ds[vb.upper()].sel(level=[850,700,500,300,250])   - clim[vb] #I have selected only the 500 and 250hpa 
    else: 
        
        clim = xr.open_mfdataset(clim_pths).mean(meaner)
        print(clim)
        ds = ds[vb.upper()] - clim[vb]
